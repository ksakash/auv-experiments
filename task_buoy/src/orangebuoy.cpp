...
void orangeBuoyDataCallback(const geometry_msgs::PointStampedPtr &_msg) {
    if (count_buoy == 0) {
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh;
    ros::Subscriber orangeBuoyDataListener = nh.subscribe("/threshold/center_coordinates", 1000, &orangeBuoyDataCallback);
    ...
    ros::Publisher yellowBuoySignalPublisher = nh.advertise<std_msgs::Bool>("/yellowBuoySignal", 1000); 
    ...
    while(ros::ok()) {
        ...
        if (buoyManager.targetAcheived()) {
            orange_buoy = false;
            std_msgs::Bool yellow_buoy_start_signal;
            yellow_buoy_start_signal.data = true;
            yellowBuoySignalPublisher.publish(yellow_buoy_start_signal);
        }
