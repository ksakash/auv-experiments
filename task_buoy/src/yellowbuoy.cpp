...
bool move_right_signal = false;
bool yellow_buoy = false;

void yellowBuoyStartSignalCallback(const std_msgs::BoolPtr &_msg) {
    if (_msg->data == true) {
        move_right_signal = true;
        yellow_buoy = true;
    }
}

void yellowBuoyDataCallback(const geometry_msgs::PointStampedPtr &_msg) {
    if (count_buoy == 0) {
        // x_coord.setReference(0);
        y_coord.setReference(0);
        move_right_signal = false;
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh;
    ros::Subscriber yellowBuoyDataListener = nh.subscribe("/threshold/center_coordinates", 1000, &yellowBuoyDataCallback);
    ros::Subscriber yellowBuoyStartSignalListener = nh.subscribe("/yellowBuoySignal", 1000, &yellowBuoyStartSignalCallback);
    ...
    ros::Publisher greenBuoySignalPublisher = nh.advertise<std_msgs::Bool>("/greenBuoySignal", 1000); 
    ...
    while(ros::ok()) {
        ...
        if (y_coord.getPWM() <= 4 || y_coord.getPWM() >= -4) {
            temp = true;
        }

        if (temp) {
            buoyManager.targetStatus(y_coord.getCurrentValue());
        }

        if (buoyManager.targetAcheived()) {
            yellow_buoy = false;
            std_msgs::Bool green_buoy_start_signal;
            green_buoy_start_signal.data = true;
            greenBuoySignalPublisher.publish(green_buoy_start_signal);
        }
